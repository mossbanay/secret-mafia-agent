...
class CodenamesEnv(ta.Env):
    def __init__(self, hardcore: Optional[bool] = False):
        self._load_word_list(hardcore=hardcore or False)
    ...
    def _load_word_list(self, hardcore: bool = False) -> None:
        ...
        self.word_list = [
            w for w, is_noun in zip(word_list, noun_mask) if is_noun and len(w) < 8
        ]

    def reset(self, num_players: int, seed: Optional[int] = None):
        assert num_players == 4, (
            f"The number of players must be exactly 4. Received {num_players}"
        )
        self.state = ta.TeamMultiPlayerState(num_players=num_players, seed=seed)
        assignments = (
            ["R"] * 9 + ["B"] * 8 + ["N"] * 7 + ["A"]
        )  # Create a list of 25 assignments: 9 Red (R), 8 Blue (B), 7 Neutral (N), and 1 Assassin (A)
        random.shuffle(
            assignments
        )  # Shuffle the assignments to randomize their placement
        self.board = {
            word: team
            for word, team in zip(random.sample(self.word_list, 25), assignments)
    ...
    def _render_player_view(
        self,
    ):  # , spymaster: bool = False, guessed_words: set = None):
        view = "Codenames Words:\n"
        for word in list(self.board.keys()):
            if self.state.current_player_id in [0, 2]:
                view += f"{word:<8} {self.board[word]} {'revealed' if word in self.state.game_state['guessed_words'] else ''}\n"  # Show the team label for spymasters
            else:
                view += f"{word:<8} {self.board[word] if word in self.state.game_state['guessed_words'] else ''}\n"
        return view
    ...
    def _prompt(self, player_id: int, game_state: Dict[str, Any]) -> str:
        ...
        if player_id in [0, 2]:
            return (
                prompt
                + f"You are Player {player_id}, the Spymaster for {'Red' if player_id == 0 else 'Blue'} team. Give a one-word clue and number."
            )
        else:
            return (
                prompt
                + f"You are Player {player_id}, the Operative for {'Red' if player_id == 1 else 'Blue'} team. Guess words based on the clue."
            )

    def _rotate_player_by_logic(
        self, done_guessing: bool = False, skip_guessing: bool = False
    ):
        match self.state.current_player_id:
            case 0:
                next_player_id = 2 if skip_guessing else 1
            case 2:
                next_player_id = 0 if skip_guessing else 3
            case 1:
                next_player_id = 2 if done_guessing else 1
            case 3:
                next_player_id = 0 if done_guessing else 3
    ...
    def step(self, action: str) -> Tuple[bool, ta.Info]:
        ...
        if self.state.current_player_id in [0, 2]:  # Spymasters give clues
            ...
            if match:
                ...
                if any(
                    word in board_word or board_word in word
                    for board_word in self.board.keys()
                ):
                ...
                self.state.game_state["remaining_guesses"] = (
                    number + 1
                ...
                terminated_by_invalid = self.state.set_invalid_move(
