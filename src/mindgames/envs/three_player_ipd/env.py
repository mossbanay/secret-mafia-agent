...
class ThreePlayerIPDEnv(ta.Env):
    ...
    def reset(self, num_players: int, seed: Optional[int] = None):
        assert num_players == 3, (
            f"Environment is hard-coded for exactly three players. Received {num_players} players on reset."
        )
        ...
        self.state.add_observation(
            message=f"─── Starting Round {game_state['round']} ───\tYou can converse freely for the next {game_state['total_conversation_rounds']} rounds.",
    ...
    def step(self, action: str) -> Tuple[bool, ta.Info]:
        ...
        match self.state.game_state["phase"]:
            case "conversation":
                self._conversation_phase(msg=action)
            case "decision":
                self._decision_phase(msg=action)
        return self.state.step()

    def _clean_message(self, msg: str) -> str:
        return re.sub(
            r"\s+", " ", msg
        )  # 1-2. strip() → remove edge whitespace; regex → collapse the rest
    ...
    def _conversation_phase(self, msg: str):
        ...
        if cid == self.state.num_players - 1:
            ...
            if gs["conversation_round"] >= gs["total_conversation_rounds"]:
                gs["phase"] = "decision"
                self.state.add_observation(
                    message=f"Chat finished for round {gs['round']}. Submit your decisions, one token per opponent: `[pid cooperate]` or `[pid defect]`.",
    ...
    def _decision_phase(self, msg: str):
        ...
        if all(gs["acted"].values()):
            ...
            if gs["round"] > gs["num_rounds"]:
                ...
                gs.update(
                    {
                        ...
                        "decisions": {
                            p: {
                                q: None for q in range(self.state.num_players) if q != p
                            }
                ...
                self.state.add_observation(
                    message=f"─── Starting Round {gs['round']} ───\tYou can converse freely for the next {gs['total_conversation_rounds']} rounds.",
                    observation_type=ta.ObservationType.GAME_MESSAGE,
                )
    ...
    def _pair_payoff(self, a: str, b: str) -> Tuple[int, int]:
        ...
        return (
            (self.S, self.T) if a == "cooperate" else (self.T, self.S)
    ...
    def _resolve_round(self):
        ...
        for p, inc in round_gain.items():
            gs["scores"][p] += inc  # accumulate scores
        message += "; ".join(
            [f"Player {p} ({gs['scores'][p]})" for p in range(self.state.num_players)]
        )
        self.state.add_observation(
            message=message + "\n", observation_type=ta.ObservationType.GAME_MESSAGE
        )
    ...
    def _end_game(self):
        ...
        if G == 1:
            reward_dict = {pid: 0.0 for pid in groups[0]}  # complete draw
        else:
        ...
        self.state.set_game_outcome(
            ...
            reason="Final scores: "
            + ", ".join(f"P{p}={scores[p]}" for p in sorted(scores))
            + f".  Ranking groups (worst→best): {groups}",
        )
