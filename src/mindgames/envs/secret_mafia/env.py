...
class VoteHandler:
    ...
    def tally(votes: Dict[int, int]) -> Optional[int]:
        ...
        top_players = [
            pid for pid, c in counts.items() if c == top_score
        ]  # All players who received the top score (could be 1 or many)
...
class SecretMafiaEnv(ta.Env):
    ...
    def reset(self, num_players: int, seed: Optional[int] = None):
        ...
        self.state.reset(
            ...
            secret_roles=self.player_roles,
        )
    ...
    def step(self, action: str) -> Tuple[bool, ta.Info]:
        ...
        phase_dispatch = {
            Phase.DAY_DISCUSSION: self._handle_discussion,
            Phase.DAY_VOTING: self._handle_day_vote,
            Phase.NIGHT_MAFIA: self._handle_mafia_vote,
            Phase.NIGHT_DOCTOR: self._handle_doctor_action,
            Phase.NIGHT_DETECTIVE: self._handle_detective_action,
        }
    ...
    def _compute_next_phase(self) -> Phase:
        doctor_alive = any(
            self.player_roles[p] == "Doctor"
            for p in self.state.game_state["alive_players"]
        )
        detective_alive = any(
            self.player_roles[p] == "Detective"
            for p in self.state.game_state["alive_players"]
        )
        match self.phase:
            case Phase.NIGHT_MAFIA:
                return (
                    ...
                    if doctor_alive
                        ...
                        if detective_alive
                        else Phase.DAY_DISCUSSION
                    )
                )
            case Phase.NIGHT_DOCTOR:
                return (
                    Phase.NIGHT_DETECTIVE if detective_alive else Phase.DAY_DISCUSSION
                )
        ...
        if self.phase == Phase.NIGHT_MAFIA:
            ...
            for p in mafia:
                self.state.add_observation(
                    to_id=p,
                    message=f"Night has fallen. Mafia, agree on a victim.\nValid targets: {', '.join(f'[{t}]' for t in targets)}",
    ...
    def _handle_doctor_action(self, pid: int, action: str):
        ...
        if target is None or target not in self.state.game_state["alive_players"]:
            ...
            if not fatal:
                ...
                self.state.made_invalid_move = (
                    False  # such that we can rotate off the player
                )
                return
    ...
    def _handle_detective_action(self, pid: int, action: str):
        ...
        self.state.add_observation(
            to_id=pid, message=result, observation_type=ta.ObservationType.GAME_MESSAGE
        )
    ...
    def _record_vote(
        ...
        *,
        ...
        broadcast_to_mafia_only=False,
    ):
        target = VoteHandler.parse(action)
        if target is None or target not in self.state.game_state["alive_players"]:
            fatal = self._mark_invalid(
                pid, "Vote not in valid format or invalid target."
            )
            if not fatal:
                return
            else:  # player was eliminated by invalid move
    ...
    def _mark_invalid(self, pid: int, reason: str):
        ...
        if fatal:
            self._eliminate_player(
                self.state.current_player_id,
                "has been eliminated by making an invalid move.",
            )
        return fatal
    ...
    def _resolve_day_votes(self):
        ...
        if target is None:
            self.state.add_observation(
                message="No consensus - nobody was eliminated.",
                observation_type=ta.ObservationType.GAME_MESSAGE,
            )
            return
    ...
    def _store_mafia_target(self):
        self.state.game_state["pending_elimination"] = VoteHandler.tally(
    ...
    def _check_win(self):
        ...
        if not mafia_alive:
            ...
            self.state.set_winners(
                player_ids=villagers, reason="All Mafia were eliminated. Village wins!"
            )
