def create_board_str(game_state: dict) -> str:
    ...
    lines = []
    lines.append(
        f"┌─ SECRET MAFIA ─────────────── Phase: {game_state.get('phase', 'Unknown'):<15} | Day: {game_state.get('day_number', 0)} ──────────────┐"
    )
    lines.append(
        "│ Player Status                                                               │"
    )
    ...
    for pid in sorted(game_state.get("player_roles", {})):
        ...
        role = role_icons.get(
            game_state.get("player_roles", {})[pid]
        )  # , player_roles[pid])
    ...
    if game_state.get("phase", "Unknown") in {"Day-Voting", "Night-Mafia"}:
        ...
        if game_state.get("votes", {}):
            for voter, target in sorted(game_state.get("votes", {}).items()):
                lines.append(f" - Player {voter} ➜ Player {target}")
        else:
            lines.append(" - No votes have been cast yet.")
    if game_state.get("to_be_eliminated", None) is not None:
        lines.append(
            f"\n🪦 Player {game_state.get('to_be_eliminated', None)} is marked for elimination."
        )
    mafia_alive = sum(
        1
        for pid in set(game_state.get("alive_players", []))
        if game_state.get("player_roles", {})[pid] == "Mafia"
    )
    village_alive = sum(
        1
        for pid in set(game_state.get("alive_players", []))
